...
public class MapperAnnotationBuilder {
    ...
    /**
     * 解析注解形式的SQL映射的核心方法
     * 调用来源是： MapperRegistry.addMapper --> 本方法
     */
    public void parse() {
        String resource = type.toString();
        // 防止资源重复解析
        if (!configuration.isResourceLoaded(resource)) {
            // 寻找类名对应的resource路径下是否有xml配置。如果有，则解析掉。实现支持注解和xml形式的混合使用
            loadXmlResource();
            // 记录资源路径
    ...
    /**
     * 根据注解获取对应的sqlSource信息
     *
     * @param annotation     注解
     * @param parameterType  参数类型
     * @param languageDriver 语言驱动
     * @param method         带有该注解的方法
     * @return 找到的sqlSource对象
     */
    private SqlSource buildSqlSource(Annotation annotation, Class<?> parameterType, LanguageDriver languageDriver,
                                     Method method) {
        // 根据注解的类型决定具体调用的方法。直接注解映射由bindSqlSourceFromStrings方法负责。简介注解映射由ProviderSqlSource方法负责
    ...
    /**
     * 处理直接注解映射。即@Select、@Update等
     * 具体处理逻辑就是将描述SQL语句的字符串拼接起来，交给languageDriver处理
     *
     * @param strings            注解中的字符串

 ...
 * 本类的子类SQL类只实现了getSelf方法，返回自身。这种设计让本类更好扩展。
 */
public abstract class AbstractSQL<T> {
    ...
    private static class SafeAppendable {
        ...
        public SafeAppendable append(CharSequence s) {
            try {
                // 主串非空或要拼接的串非空，将标识属性empty置为false
                if (empty && s.length() > 0) {
                    empty = false;
                }
                // 拼接字符串
    ...
    private static class SQLStatement {
        ...
        // 是否去重
        boolean distinct;
        // 结果偏移量，limit 10, 20的10
        String offset;
        // 结果总数约束，limit 10, 20的20
        String limit;
        // 结果约束策略
        ...
        private String selectSQL(SafeAppendable builder) {
            ...
            joins(builder); // join操作较为复杂，抽取了方法

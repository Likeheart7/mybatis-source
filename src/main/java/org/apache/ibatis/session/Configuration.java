...
public class Configuration {
    ...
    // 类型处理器注册表，大多是内置的，可以通过<typeHandler>标签补充
    protected final TypeHandlerRegistry typeHandlerRegistry = new TypeHandlerRegistry(this);
    // 类型别名表，可以通过<typeAliases>标签补充
    ...
    public ParameterHandler newParameterHandler(MappedStatement mappedStatement, Object parameterObject, BoundSql boundSql) {
        ...
        // 将参数处理器交给拦截器进行替换，以便拦截器链中的拦截器能注入行为
    ...
    /**
     * 一个HashMap的子类，具有以下特点
     * 1. 不允许覆盖，如果写入的key已存在，直接报错
     * 2. 如果写入的key包含"."，如"com.chenx.pojo.className"，则会分别以className和com.chenx.pojo.className作为key存入两次
     *
     * @param <V>
     */
    protected static class StrictMap<V> extends HashMap<String, V> {
        ...
        /**
         * 向StrictMap中放入键值对，如果key中有“.”，会根据key和key最后一个单词添加两组键值对
         *
         * @param key   key with which the specified value is to be associated
         * @param value value to be associated with the specified key
         * @return 旧值，如果没有就是null，因为不能覆盖，所以这里一直是null
         */
        @Override
        @SuppressWarnings("unchecked")
        public V put(String key, V value) {
            // 如果map中已经有这个key，直接抛出异常
            if (containsKey(key)) {
                throw new IllegalArgumentException(name + " already contains value for " + key
                        + (conflictMessageProducer == null ? "" : conflictMessageProducer.apply(super.get(key), value)));
            }
            // 如果key中有“。”，根据最后一个.后的字符串也添加一组键值对
            if (key.contains(".")) {
                final String shortKey = getShortName(key);
                if (super.get(shortKey) == null) {
                    super.put(shortKey, value);
                } else { // 如果这个短key已经存在，就给value包装一下，表示有歧义
